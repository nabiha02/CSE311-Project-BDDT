...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $rdate   = $_POST['R_Start_Date'];
    $budget = $_POST['R_Budget'] ?: NULL;
    $cost   = $_POST['R_Cost'] ?: NULL;
    $status = $_POST['R_Status'];
    $date   = $_POST['R_Publish_Date'];
    $sector = $_POST['Sector_ID'];

    // Check for duplicate research title
    $check = $conn->prepare("SELECT research_ID FROM research WHERE R_title=? LIMIT 1");
    $check->bind_param("s", $title);
    $check->execute();
    $res = $check->get_result();
    if($res->num_rows > 0){
        die("Research with this title already exists.");
    }

    // Insert research
    $sql = $conn->prepare("INSERT INTO research (R_title, R_Description, R_Start_Date, R_Budget, R_Cost, R_Status, R_Publish_Date, Sector_ID) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    $sql->bind_param("ssssssss", $title, $desc, $rdate, $budget, $cost, $status, $date, $sector);
    $sql->execute();
    $research_id = $sql->insert_id;
    ...
    if(!empty($_POST['I_Name'])){
        foreach($_POST['I_Name'] as $idx => $iname){
            ...
            $amount = $_POST['R_Contribution_Amount'][$idx] ?: NULL;
            $agreement = $_POST['R_Agreement_Date'][$idx] ?: NULL;
            ...
            // Link investor to research
            if($amount){
                $q2 = $conn->prepare("INSERT INTO research_funded_by_investor (research_ID, Investor_ID, R_Contribution_Amount, R_Agreement_Date) VALUES (?, ?, ?, ?)");
                $q2->bind_param("iiss", $research_id, $inv_id, $amount, $agreement);
                $q2->execute();
            }
        }
    }
    ...
    $loc_id = $ins_loc->insert_id;
            ...
            // Link research to location
            $q3 = $conn->prepare("INSERT INTO research_occurs_at_location (research_ID, Location_ID) VALUES (?, ?)");
            $q3->bind_param("ii", $research_id, $loc_id);
            $q3->execute();
        }
    }
...
<div class="form-wrapper">
    <div class="form-box">
        ...
        <form method="POST">
            ...
            <textarea name="R_Description" rows="4"></textarea>

            <label>Start Date</label>
            <input type="date" name="R_Start_Date">

            <label>Budget</label>
            <input type="text" name="R_Budget">

            <label>Cost</label>
            <input type="text" name="R_Cost">

            <label>Publish Date</label>
            <input type="date" name="R_Publish_Date" required>
            ...
            <div id="investors-wrapper">
                <div class="investor-entry">
                    ...
                    <input type="text" name="R_Contribution_Amount[]" placeholder="Amount">

                    <label>Agreement Date</label>
                    <input type="date" name="R_Agreement_Date[]">
